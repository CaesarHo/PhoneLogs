...
package com.caesar.phonelogs.utils.pinyinutil;
...
public class HanziToPinyin {
    private static final String TAG = "BTApp.HanziToPinyin";
	...
	/** Special China surname and the corresponding pinyin */
	private static final char[] SPECIAL_CHN_SURNAME = { '\u4e50', '\u89e3',
			                                        '\u5355', '\u4ec7', 
			                                        '\u851a', '\u66fe', 
			                                        '\u4e5c', '\u67e5',
			                                        '\u4f74', '\u6fb9' };
	private static final String[] SPECIAL_CHN_SURNAME_PINYIN = { "YUE", "XIE",
			                                                 "SHAN", "QIU", 
			                                                 "YU", "ZENG", 
			                                                 "NIE", "ZHA", 
			                                                 "NAI", "TAN" };
	...
	private Token getSpecialToken(Character character) {
		Token token = new Token();
		final String letter = Character.toString(character);
		token.source = letter;
		token.type = Token.PINYIN;
		...
		StringBuilder pinyin = new StringBuilder();
		for (int i = 0; i < SPECIAL_CHN_SURNAME.length; i++) {
			if (character == SPECIAL_CHN_SURNAME[i]) {
				pinyin.append(SPECIAL_CHN_SURNAME_PINYIN[i]);
				token.target = pinyin.toString();
				return token;
			}
		}
    ...
    public ArrayList<Token> get(final String input) {
        ...
        for (int i = 0; i < inputLength; i++) {
            ...
            if (character == ' ') {
				...
				if (i == 0) {
					Token t = getSpecialToken(character);
					if (null != t) {
						tokens.add(t);
						continue;
					}
