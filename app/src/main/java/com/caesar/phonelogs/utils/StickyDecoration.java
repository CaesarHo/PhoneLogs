...
public class StickyDecoration extends RecyclerView.ItemDecoration {
    ...
    public StickyDecoration(Context context, ArrayList<CallInfoLog> callInfoLogs) {
        ...
        this.callInfoLogs = callInfoLogs;
    }
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            int position = parent.getChildAdapterPosition(view);
            String textLine = callInfoLogs.get(position).getCallTime();
            if (isFirstInGroup(position)) {
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        String text = callInfoLogs.get(position).getCallTime();
    ...
    private boolean isFirstInGroup(int position) {
        ...
        if (position == 0) {
            ...
            if (callInfoLogs.get(position).getCallTime().
                    equals(callInfoLogs.get(position - 1).getCallTime())) {
                isFirst = false;
            } else {
                isFirst = true;
            }
